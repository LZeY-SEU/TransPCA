import cupy as cp  # implace cupy as numpy if neccessary
from pca_utils_GPU import *
...
class TransPCA:
    def __init__(self, target, source_list, cov='covariance', 
        ...
        self.source_list = [cp.asarray(source) for source in source_list]
        self.cov = cov
        self.r0s = r0s
        ...
        if nk_list is not None:
            self.nk_list = cp.array(nk_list)
        else:
            self.nk_list = cp.array([source.shape[0] for source in self.source_list])
        ...
        if cov == 'covariance':
            ...
            self.target_cov = cp.asarray(sample_Kendall_tau(self.target)) 
            self.source_cov_list = [cp.asarray(sample_Kendall_tau(source)) for source in self.source_list]
        else:
        ...
        self.selected_k_list = cp.array([1] * len(self.source_cov_list))
    ...
    def fit(self, max_iteration=10):
        ...
        for t in range(max_iteration):
            ...
            if cp.array_equal(self.selected_k_list, previous_selected_k_list):
    ...
    def get_PCA_Us(self):
        ...
        self.Us = cp.linalg.svd(Sigma_PCA)[0][:, :self.r0]
    ...
    def get_GB_Us(self):
        ...
        # Perform eigen decomposition to extract the top r0 eigenvectors
        self.Us = cp.linalg.svd(Sigma_GB)[0][:, :self.r0]
    ...
    def select_k(self):
        ...
        for k, Uk in enumerate(self.Uk_list):
            proj_error = cp.trace((cp.eye(self.p) - Uk @ Uk.T) @ self.Urs @ self.Urs.T) 
        ...
        self.selected_k_list = cp.array(selected_k_list)
    ...
    def optional_finetuning(self):
        ...
        for r in range(1, self.r0 + 1):
            ...
            proj_error = cp.trace((cp.eye(self.p) - self.U0 @ self.U0.T) @ Ur @ Ur.T) 
        ...
        if self.r0s is not None: #overwrite
            ...
            Prs_perp = cp.eye(self.p) - self.Urs @ self.Urs.T
            Uf = cp.linalg.svd(Prs_perp @ self.target_cov @ Prs_perp)[0][:, :self.r0 - self.rs]
            self.finetuned_U0 = cp.hstack((self.Urs, Uf))  
...
class cvTransPCA(TransPCA):
    def __init__(self, target, source_list, cov='covariance',
        ...
        self.delta_grid = cp.asarray(cp.logspace(-3, 0, num=10)) if delta_grid is None else cp.asarray(delta_grid)
        self.tau_grid = cp.asarray(cp.logspace(-3, 0, num=10)) if tau_grid is None else cp.asarray(tau_grid)
    ...
    def fit(self, max_iteration=10):
        ...
        total_norm = cp.linalg.norm(self.target, 'fro')**2
        ...
        for delta_val in self.delta_grid:
            for tau_val in self.tau_grid:
                ...
                for train_idx, val_idx in kf.split(cp.asnumpy(self.target)): 
                    X_train, X_val = self.target[train_idx], self.target[val_idx]

                    temp_model = TransPCA(X_train, [cp.asarray(source) for source in self.source_list],
                    ...
                    P = cp.asarray(temp_model.finetuned_U0) @ cp.asarray(temp_model.finetuned_U0).T
                    proj_val = P @ cp.asarray(X_val).T
                    sum_metric += cp.linalg.norm(proj_val, 'fro')**2
