...
def sample_Kendall_tau(X):
    ...
    For each pair of samples (i, j), the function computes:
        diff = X[i, :] - X[j, :]
        Contribution = (diff.reshape(p, 1) @ diff.reshape(1, p)) / ||diff||^2
    The final matrix K is the average of these contributions, scaled by 2/(n*(n-1)).
    """
    n, p = X.shape
    TK = np.zeros((p, p))
