...
def select_pca_dimension(cov_matrix, criterion='eig_ratio', variance_ratio=0.95, r_max=None):
    ...
    pca = PCA()
    pca.fit(cp.asnumpy(cov_matrix)) 
    
    # Eigenvalues of the covariance matrix
    eigenvalues = cp.asarray(pca.explained_variance_) 
    ...
    if criterion == 'variance_ratio':
        ...
        r = cp.where(cumulative_variance >= variance_ratio)[0][0] + 1  # +1 to match index
    
    elif criterion == 'AIC' or criterion == 'BIC':
        # Case when p < n (low-dimensional case)
        if p < n:
            def calc_bar_ell(j):
                return cp.mean(eigenvalues[j+1:])
            
            AIC_values, BIC_values = [], []
            for j in range(p - 1):
                bar_ell = calc_bar_ell(j)
                sum_log_ell = cp.sum(cp.log(eigenvalues[j+1:]))
                
                AIC_j = (p - j - 1) * cp.log(bar_ell) - sum_log_ell - ((p - j - 1) * (p - j + 2)) / n
                BIC_j = (p - j - 1) * cp.log(bar_ell) - sum_log_ell - ((p - j - 1) * (p - j + 2)) / (2 * n) * cp.log(n)
                
                AIC_values.append(AIC_j)
                BIC_values.append(BIC_j)

            r = cp.argmin(AIC_values) + 1 if criterion == 'AIC' else cp.argmin(BIC_values) + 1
        
        # Case when p >= n (high-dimensional case)
        else:
            def calc_bar_ell(j):
                return cp.mean(eigenvalues[j+1:n])
            
            AIC_values, BIC_values = [], []
            for j in range(n - 1):
                bar_ell = calc_bar_ell(j)
                sum_log_ell = cp.sum(cp.log(eigenvalues[j+1:n]))
                
                AIC_j = (n - 1 - j) * cp.log(bar_ell) - sum_log_ell - ((n - j - 2) * (n - j + 1)) / p
                BIC_j = (n - 1 - j) * cp.log(bar_ell) - sum_log_ell - ((n - j - 2) * (n - j + 1)) / (2 * p) * cp.log(p)
        ...
        r = cp.argmax(eig_ratios[:r_max]) + 1  # +1 to match indexing
...
def compute_pca(cov_matrix, r):
    ...
    U = cp.linalg.svd(cov_matrix)[0][:, :r]
    return U
